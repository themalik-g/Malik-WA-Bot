...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, { text: '📢 *Join our Channel:*\nhttps://chat.whatsapp.com/FfJZtyvL1PM46pLmInoHcZ' }, { quoted: message });
                return;
        ...
        if (userMessage.startsWith('.')) console.log(`📝 Command used in ${isGroup ? 'group' : 'private'}: ${userMessage}`);
