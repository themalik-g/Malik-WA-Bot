...
async function reminiCommand(sock, chatId, message, args) {
    try {
        ...
        const reminiApiKey = process.env.REMINI_API_KEY;
        if (!reminiApiKey) {
            return sock.sendMessage(chatId, {
                text: '❌ Remini API Key is not configured.'
            }, { quoted: message });
        }

        // Call the Remini API
        const apiUrl = `https://api.princetechn.com/api/tools/remini?apikey=${reminiApiKey}&url=${encodeURIComponent(imageUrl)}`;
